...
# SURVDATpull comparison

Digging into differences in the fall Bigelow survey data across different pulls oof `survdat`
...
trawldat_combined %>% 
  ...
  labs(subtitle = "numlen to abundance conversion Value",
...
plot_stations <- function(combined_data, species_choice){
  ...
  station_plot <- ggplot(species_numlens, aes(est_year, n_stations, color = survdat_source)) +
    geom_line() +
    facet_wrap(~season) +
    labs(x = "", y = "total stations")
  
  numlen_plot <- ggplot(species_numlens, aes(est_year, sum_numlen, color = survdat_source)) +
    geom_line() +
    facet_wrap(~season) +
    labs(x = "", y = "total numlen abundance", caption = paste0(species_choice, " only"))
...
plot_biomass <- function(combined_data, species_choice){
  ...
  abund_plot <- ggplot(species_abund_bio, aes(est_year, abundance, color = survdat_source)) +
    geom_line() +
    facet_wrap(~season) +
