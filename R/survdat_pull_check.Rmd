...
pull_pivot_plot <- function(target_comname = "haddock", comparison = c("Feb 2021", "Jan 2021", "2016-2019")){
  ...
  yr_ratios <- annual_totals %>% 
    ...
    mutate(
      b_ratio_febtofeb  = `biomass_Feb 23 2021` / `biomass_Feb 2021`,
      b_ratio_febto2021 = `biomass_Feb 23 2021` / biomass_2021,
      b_ratio_febto2019 = `biomass_Feb 23 2021` / biomass_2019,
      a_ratio_febtofeb  = `abundance_Feb 23 2021` / `abundance_Feb 2021`,
  ...
  caption <- switch(
    comparison,
    "Feb 2021" = "It was noticed with the Feb 18 pull that there were a handful of cases where the
    abundance conversion numbers were incorrectly hard-coded, this should now be fixed",
  ...
  geom_line(aes(est_year, !!ycol_b), linetype = 2) +
  #geom_point(aes(est_year, !!ycol_b)) +
  facet_wrap(~season) + 
  labs(x = "", 
       y = ylab_b,
  ...
  abund_ratio <- ggplot(yr_ratios, aes(est_year)) +
    geom_line(aes(est_year, !!ycol_a), linetype = 2) +
    #geom_point(aes(est_year, !!ycol_a)) +
    facet_wrap(~season) + 
    labs(x = "", 
         y = ylab_a,
...
```{r}
pull_pivot_plot(target_comname = "haddock", comparison = "Feb 2021")
```
...
```{r}
pull_pivot_plot(target_comname = "atlantic cod", comparison = "Feb 2021")
```
...
```{r}
pull_pivot_plot(target_comname = "american lobster", comparison = "Feb 2021")
```
