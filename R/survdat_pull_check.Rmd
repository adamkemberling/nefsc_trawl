...
species_totals <- trawldat_combined %>% 
  filter(survdat_source %in% c("Jan 2021", "Feb 2021", "Feb 23 2021"),
         est_year >= 2009,
         comname != "0") %>% 
    ...
    map_dfr(function(species_dat){
      species_dat %>% 
        ...
        mutate(feb_23_bratio = `biomass_Feb 23 2021` / `biomass_Jan 2021`,
               feb_18_bratio = `biomass_Feb 2021`    / `biomass_Jan 2021`,
               feb_23_aratio = `abundance_Feb 23 2021`  / `abundance_Jan 2021`,
               feb_18_aratio = `abundance_Feb 2021`     / `abundance_Jan 2021`) %>% 
          group_by(comname, svspp) %>% 
          summarise(`Biomass Conv. Factor -  Feb, 23 : Jan, 21` = round(1 / mean(feb_23_bratio), 3),
                    `Biomass Conv. Factor -  Feb, 18 : Jan, 21` = round(1 / mean(feb_18_bratio), 3),
                    `Abundance Conv. Factor Feb, 23 : Jan, 21`  = round(1 / mean(feb_23_aratio), 3),
                    `Abundance Conv. Factor Feb, 18 : Jan, 21`  = round(1 / mean(feb_18_aratio), 3),
                    .groups = "keep") %>% 
          ungroup() 
      }) %>% arrange(svspp)

# Fancy table  
DT::datatable(species_ratios, rownames = FALSE,
              caption = "Conversion Factors for SVSPP numbers >= 100 Likely to mismatch Miller Paper.")
```
