...
bio_2 <- seasonals %>% 
  ggplot(aes(est_year, lw_biomass_kg, color = season)) +
    geom_line() +
    scale_y_continuous(labels = scales::comma_format()) +
    labs(x = "", y = "Survey Biomass (kg)", caption = "Biomass from length-weight regressions.")
...
Area stratified totals and catch/tow for abundance and biomass are calculated according to the following formulas in accordance with code found [here, courtesy of Sean Lucey](https://github.com/slucey/RSurvey/blob/master/Survdat/R/survdat_functions.R)
...
abund_2 <- ann_means %>% 
  ...
  labs(x = "", y = "Area Extrapolated Abundance \n(millions)",
       caption = "Stratification weights from nmfs trawl stratum, not ecological production units.") +
...
lw_strat_1 <- ann_means %>% 
  ggplot() +
    geom_line(aes(est_year, lw_strat_biomass_s/1000000, 
                  color = season, linetype = "NMFS Stratum Weights")) +
    geom_line(aes(est_year, lw_strat_biomass_epu/1000000, 
                  color = season, linetype = "EPU Area Weights")) +
    scale_color_manual(values = "black") +
    scale_y_continuous(labels = scales::comma_format()) +
    facet_wrap(~`Research Vessel`, scales = "free") +
    labs(x = "", y = "Area Expanded Biomass\nl-W Regressions (million kg)") 
...
As for the shipboard measurements of biomass `survdat$biomass`, the Biomass below is a function of the total annual abundance, expanded out to total survey area, multiplied by the biomass as recorded in fscs. This "biomass" is the total biomass for a given species at a station, distributed evenly across *n* groups for the different lengths encountered. This is **not** distributed equally by the number of fish at each length, just the number of *n* groups
...
strat_1 <- ann_means %>% 
  ggplot() +
    geom_line(aes(est_year, fscs_strat_biomass_s/1000000, 
                  color = season, linetype = "NMFS Stratum Weights")) +
    geom_line(aes(est_year, fscs_strat_biomass_epu/1000000, 
                  color = season, linetype = "EPU Area Weights")) +
    scale_color_manual(values = "black") +
    scale_y_continuous(labels = scales::comma_format()) +
    facet_wrap(~`Research Vessel`, scales = "free") +
...
strat_2 <- seasonals %>% 
  ggplot(aes(est_year, fscs_strat_biomass_s/1000000, color = season)) +
    geom_line() +
    scale_y_continuous(labels = scales::comma_format()) +
  facet_wrap(~`Research Vessel`, scales = "free") +
    labs(x = "", 
...
Biomass per station in the following figures represents the total biomass divided by the effort in number of stations. Adjustments for the 2008 vessel change are assumed to be corrected for in the data. There has been no correction for differences in stratum area in these figures, only station effort.


```{r, fig.height = 8}
cpue_1 <- ann_means %>% 
  ggplot() +
    geom_line(aes(est_year, lw_biomass_per_station, color = season, linetype = "All Species")) +
    geom_line(data = haddock_ann, 
              aes(est_year, lw_biomass_per_station, color = season, linetype = "Haddock Removed")) +
    scale_color_manual(values = "black") +
    labs(x = "", y = "LW Biomass / Station\n(kg)")
...
guild_plots <- weights_20 %>% 
  ...
  imap(function(guild, guild_name){
    ...
    p1 <- ggplot(year_df, aes(length, fct_rev(year))) +
      ...
      theme(legend.position = "none") #+facet_wrap(`Research Vessel` ~ ., ncol = 1, scales = "free_y")
    ...
    p2 <- ggplot(year_df, aes(`weight (kg)`, fct_rev(year))) +
      ...
      theme(axis.text.y = element_blank(),
            axis.ticks.y = element_blank()) #+facet_wrap(`Research Vessel` ~ ., ncol = 1, scales = "free_y")
...
weights_20 %>% 
  ...
  ggplot(aes(est_year, abundance, color = season)) +
  geom_point() +
  facet_wrap(~`Research Vessel`, scales = "free_x") +
  labs(x = "", y = "Seasonal Abundance")
...
l2 <- ggplot(bin_summs, aes(lenbin2, l_norm, fill = svvessel)) +
    ...
    facet_wrap(~svvessel) +
    scale_fill_gmri() +
    labs(x = "log2 Length Bins", y = "Normalized Abundance")
...
wt2 <- ggplot(bin_summs, aes(wtbin, wtbin_norm, fill = svvessel)) +
    ...
    facet_wrap(~svvessel) +
    scale_fill_gmri() +
    labs(x = "log2 Weight Bins", y = "Normalized Abundance")
...
**NOTE:**
The resulting analysis has been removed to shorten the length of this markdown.
