...
# SizeSpectra Results on Stratified Mean Catch
withr::with_dir(rprojroot::find_root('_targets.R'), tar_load(strat_mean_mle_ss))     
...
ggplot(regional_lengths, 
  ...
  labs(y = "Total Predicted Abundance - Full Survey Area", 
       title = "Abundance",
       subtitle = "Stratified Total Abundance", 
       x = "", 
       fill = "Length Bin") 
...
ggplot(regional_lengths, 
  ...
  scale_fill_gmri() +
  scale_y_continuous(labels = comma_format()) +
  labs(y = "Stratified Total Biomass (kg) - Full Survey Area",  
       title = "Biomass",
       subtitle = "Stratified Total Biomass", 
       x = "", 
...
seasonal_totals <- nefsc_size_bins %>% 
  ...
  summarise(total_survey_catch = sum(numlen),
            total_lw_bio = sum(sum_weight_kg),
            total_strat_abund = sum(strat_total_abund_s),
            total_strat_lw_bio = sum(strat_total_lwbio_s),
            .groups = "drop")
```
...
# Community Size Spectra Results {.tabset .tabset-pills}
...
## Biomass Data to Match Size Spectrum Analysis {.tabset}
