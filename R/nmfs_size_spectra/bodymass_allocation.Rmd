...
# Pull Data for some specific subset
t_data <- nefsc_size_bins %>% 
  filter(survey_area == area_3d) %>% 
  split(.$Year)

# Assign bins
t_year <- map(t_data, assign_log10_bins)

# Aggregate by bin
t_agg <- map_dfr(t_year, aggregate_l10_bins, .id = "Year") %>% 
  mutate(Year = as.numeric(Year),
         log10_bins = as.numeric(log10_bins),
...
library(plotly)
yourdata <- select(t_agg, log10_bins, Year, norm_strat_abund)

pivot_wider(yourdata, names_from = Year, values_from = norm_strat_abund)

# defining data in the vector
x <- yourdata$norm_strat_abund
  
# defining row names and column names
rown <- sort(unique(yourdata$log10_bins))
coln <- sort(unique(yourdata$Year))
...
# Making matrix
m2 <- matrix(nrow = max(yourdata$log10_bins) - (min(yourdata$log10_bins)-1), 
             ncol = max(yourdata$Year) - (min(yourdata$Year)-1))
dim(m2)

rownames(m2) <- sort(unique(yourdata$log10_bins))
colnames(m2) <- sort(unique(yourdata$Year))
m2[as.matrix(yourdata[1:2])] <- yourdata$norm_strat_abund
m2

spectrum_mat <- as.matrix(xtabs(norm_strat_abund~log10_bins + Year, data = yourdata))
plot_ly(
  ...
  layout(
    ...
    scene = list(
      ...
      yaxis = list(title = "Log10( BodyMass )"),
      zaxis = list(title = "Log10( Normalized Abundance) "),
...
# # Plot 3d using geom_contour
# (contour_3d <- t_agg %>% 
#   ggplot(aes(log10_bins, 
#              Year, 
#              z = norm_strat_abund,
#            color = norm_strat_abund)) +
#   geom_contour_filled() +
#   facet_wrap(~ season) +
#   labs(x = "Log10 Bodymass (g)",
#        fill = "Log10 Stratified Abundance Density"))
# 
...
# # 3d Surfaces using raster
# gg_3d <- t_agg %>% 
#   ggplot(aes(log10_bins, Year, 
#            z = norm_strat_abund,
#            color = norm_strat_abund)) +
#   #geom_contour_filled() +
#   geom_raster() +
...
#                        breaks = c(0:6), na.value = "transparent") +
#   labs(x = "Individual Bodymass (g)",
#        fill = "Log10 Stratified Abundance Density") +
#   guides(fill = guide_colorsteps(title.hjust = 0.5, 
#                                  title.position = "top", barwidth = unit(3.5, "in"))) +
#   theme(legend.position = "bottom")
# 
# # render it
# plot_gg(gg_3d, 
...
# Would be better if prepared for using geom_polygon
# need to create cell edge coordinates based on year (y) and log10 bins (x) so that it is four points instead of the center point. The fill would then just be the same
# the challenge would be how to make edges meet left, right, top, bottom
