...
all_parms <- lm(`ISD Exponent` ~ SST * GSI + `Commercial Landings` + decade, data = gom_df)
...
results <- dredge(all_parms,# subset= !(regime && decade), 
                  m.lim = c(1,3), fixed = c("(Intercept)"))
# subset(results, delta == 0)

# The "best" model
bmod <- lm(`ISD Exponent` ~ decade + `Commercial Landings`, data = gom_df)
...
# fit model with all parameters
all_parms <- lm(`ISD Exponent` ~ SST * GSI + `Commercial Landings` + decade, data = gb_df)
...
# fit model with all parameters
all_parms <- lm(`ISD Exponent` ~ SST * GSI + `Commercial Landings` + decade, data = mab_df)
...
# fit model with all parameters
all_parms_binned <- lm(`Spectra Slope` ~ SST * GSI + `Commercial Landings` + decade  + regime, 
                       data = slope_df)
