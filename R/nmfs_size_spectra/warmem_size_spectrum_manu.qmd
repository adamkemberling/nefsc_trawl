...
# Join them up
clim_drivers <- bind_rows(list(nao, gsi)) %>% 
  filter(EPU == "All")
...
area_df <- data.frame(area = c("Gulf of Maine", "Georges Bank", "Southern New England", "Mid-Atlantic Bight"),
                      survey_area = c("GoM", "GB", "SNE", "MAB"))

# Reshape to match the others
...
# Can we correlate them with the area mismatch?
...
gg_rho_tidy <- gg_rho %>% 
  mutate(
    ...
    driver_var = ifelse(str_detect(yvar, "osc|pca|landings|index"), T, F),
    area = case_when(
...
gg_rho_tidy %>% 
  ...
  geom_tile() +
  facet_grid(area~driver_type, scales = "free_x") +
  scale_fill_distiller(palette = "RdBu", limits = c(-1, 1), breaks = c(-1,0,1)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),
        strip.text.y = element_text(angle = 0),
        legend.position = "bottom") +
  labs(x = "Driver", y = "Response Variable",
       fill = "Correlation Coefficient",
       title = "Correlation Between Size Spectra Response and Drivers",
       subtitle = "Correlation for Concurrent Years")
```
