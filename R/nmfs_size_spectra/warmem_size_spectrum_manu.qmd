...
gom_df <- filter(regression_df, spectra_area == "Gulf of Maine") %>% 
  mutate(regime = ifelse(year < 2010, "regime_1", "regime_2" ))

# fit model with all parameters
all_parms <- glm(`ISD Exponent` ~ SST * GSI + `Commercial Landings` + decade  + regime, 
                 data = gom_df)
...
# results <- dredge(all_parms, subset= !(regime && decade), m.lim = c(1,3), fixed = c("(Intercept)"))
...
bmod <- glm(`ISD Exponent` ~ decade + `Commercial Landings`, data = gom_df)
# plot(residuals(bmod),type="b"); abline(h=0,lty=3)
# acf(residuals(bmod))


# Use Model Selection Manually
