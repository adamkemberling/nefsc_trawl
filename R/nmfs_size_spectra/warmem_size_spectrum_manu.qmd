...
plot_period_correlations <- function(x, label) {
  ...
  labs(x = "Correlated Feature", y = "Bodymass Spectra Coefficient",
...
ccf_plot_data %>% 
  ...
  geom_text(
    data = filter(ccf_plot_data, sig_flag),
    aes(lag, y = acf, label = lag, vjust = ifelse(acf < 0, 1,-1)), color = "black") +
...
isd_area_mods <- regression_df %>% 
  split(.$spectra_area) %>% 
  map(function(area_df){
    
    # Define the models here
    area_lm <- lm(`ISD Exponent` ~ SST * `Commercial Landings` * GSI, 
    # area_lm <- lm(`Spectra Slope` ~ SST * `Commercial Landings` * GSI, 
                  data = area_df )
    
    return(
      list(
        "data" = augment(area_lm, drop_na(area_df), interval = "confidence"),
        "params" = tidy(area_lm),
        "summary" = glance(area_lm)
      )
...
# Pull them into their own dataframes:
isd_data <- map_dfr(isd_area_mods, pluck, "data", .id = "survey_area")
isd_params <- map_dfr(isd_area_mods, pluck, "params", .id = "survey_area")
isd_summs <- map_dfr(isd_area_mods, pluck, "summary", .id = "survey_area")
```


```{r}
#| label: isd-model-results
# Do a conditional table
isd_params %>% 
  mutate(across(where(is.numeric), round, 3)) %>% 
  group_by(survey_area) %>% 
  gt(rowname_col = "term") %>% 
  gt::tab_header(title = "Regressions Summary", subtitle = "Independent Regional Models, interactions between predictors.") %>% 
  gt::tab_style(
    style = list(
...
## Normalized Body Mass Spectra Regressions
