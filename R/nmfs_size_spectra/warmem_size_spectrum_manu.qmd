...
# # Refine the dredge
results <- dredge(all_parms, m.lim = c(1,4), fixed = c("(Intercept)"))
...
# fit model with all parameters
all_parms <- lm(`ISD Exponent` ~ SST + GSI + `Commercial Landings` + zp_small + zp_large, data = mab_df)
...
#grab best model
# subset(results, delta == 0)
bmod <- lm(`ISD Exponent` ~ zp_small, data = mab_df)
...
```{r}
all_parms <- lm(`ISD Exponent` ~ SST + GSI + `Commercial Landings` + zp_small + zp_large, 
                data = mab_df)
