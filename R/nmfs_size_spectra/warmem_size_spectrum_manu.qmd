...
fgroup_all  <- fgroup_all %>% map(function(x){
  mutate(x, survey_area = "Northeast Shelf")})
...
# NA's in 2011 for GB
#landings_idx %>% filter(area == "Georges Bank") %>% distinct(year) %>% pull()
...
all_drivers <- bind_rows(
  list(clim_idx,landings_idx,sst_idx,zp_idx,ss_idx))


# At this stage, no NA's landings in 2011
#all_drivers %>% filter(area == "Georges Bank", var == "landings") %>% distinct(year) %>% pull()
```

# ISSUE: driver-pivot introducing NA's
...
drivers_wide <- all_drivers %>% 
  filter(
    ...
    year <= 2019) %>% 
  mutate(
    id = str_replace_all(str_c(area, "_", var), " ", "_")) %>% 
...
#drivers_scaled %>% write_csv(here::here("data/env_drivers_scaled.csv"))
```
...
# Export for use in spectra_driver_modeling.R
#drivers_wide %>% write_csv(here::here("data/env_drivers_raw.csv"))
...
# Last wrangling:
# columns for: year, slope, isd, spectra region, driver_region, GSI, sst_anom, landings
...
# Isolate Regions
gom_df <- filter(regression_df, spectra_area == "Gulf of Maine") %>% drop_na()
gb_df <- filter(regression_df, spectra_area == "Georges Bank") %>% drop_na()
sne_df <- filter(regression_df, spectra_area == "Southern New England")  %>% drop_na()
...
# plot(gb_cpt, main = "Georges Bank, Break Point Analysis")
# plot(sne_cpt, main = "Southern New England, Break Point Analysis")
# plot(mab_cpt, main = "Mid-Atlantic Bight, Break Point Analysis")
