...
area_df <- data.frame(area = c("Gulf of Maine", "Georges Bank", "Southern New England", "Mid-Atlantic Bight", "All"),
...
gg_rho_tidy <- gg_rho %>% 
  mutate(
    ...
    driver_var = ifelse(str_detect(yvar, "osc|pca|landings|index|anom"), T, F),
    area = case_when(
...
only_driver_response <- gg_rho_tidy %>% 
  ...
  mutate(
    ...
    yvar = factor(yvar, levels = c(
      ...
      "Gulf of Maine cpr pca 2","All gulf stream index",
...
only_driver_response %>% 
  ...
  labs(x = "Driver", y = "Bodymass Spectra Coefficient",
       fill = "Correlation Coefficient",
