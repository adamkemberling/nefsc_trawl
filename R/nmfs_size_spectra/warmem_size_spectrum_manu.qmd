...
#--------------------
# 4. Zooplankton
zp_idx <- zp_index %>% pivot_longer(cols = c(zp_small, zp_large), names_to = "var", values_to = "value") %>% rename(area = survey_area)
...
Cross correlation function estimates were used to look at correlation changes at one year intervals to investigate potential lag effects between the hypothesized drivers and the community's exponent of size spectra. CCF estimates were performed between the dependent variables of the size spectra slope, size spectra intercept, and individual size distribution exponent with the independent variables of the annual gulf stream index, and the corresponding regional commercial landings and sea surface temperature anomalies. CCF estimates were primarily done for exploratory purposes, and to inform potential needs to account for temporal autocorrelations.
...
### Slope Breakpoint Analyses

In addition to CCF estimates, we also investigated whether the timeseries of regional size spectra slopes showed support for regime-shift(s) using breakpoint analyses. These exploratory analyses were performed using the EnvCpt package in R, and tested each region's timeseries independently using 12 common candidate models. These 12 model candidates test whether there is evidence for a change point in any combination in slope, mean, and autoregressive structure of 1 to 2 time steps. The 12 candidate models can then be compared against one another using Aikike Information Criterion to identify between model support for whichever structure best models the shape in the underlying timeseries.
...
Following exploratory analyses with CCF functions and breakpoint analyses, regression models were developed and tested independently for each region. Candidate models were evaluated using AIC to determine the most parsimonious models for each regional system. Potential drivers included in the models were the annual Gulf Stream Index, the annual regionally averaged SST anomaly value, regionally averaged small and large copepod indices, and the scaled annual commercial landings from that region. Gulf stream index, zooplankton indices, and SST anomalies were not standardized prior to use in model fitting and evaluation as the two former are already an index value and the latter is scaled to a thirty-year climatological reference period.
...
regression_df <- drivers_scaled %>% select(!ends_with(c("zp_small","zp_large")))
...
regression_df <- left_join(regression_df, rename(zp_index, spectra_area = survey_area), 
...
```{r}
#| eval: true
#| label: predictor-relationships

# Do predictor evaluations against one another
# SST x Landings
ggplot(regression_df, aes(SST, `Commercial Landings`, color = decade)) +
  geom_point() + facet_wrap(~spectra_area) +
  scale_color_gmri() +
  labs(title = "SST x Commercial Landings")
```
...
```{r}
# SST x GSI
ggplot(regression_df, aes(SST, GSI, color = decade)) +
  geom_point() + facet_wrap(~spectra_area) +
  scale_color_gmri() +
  labs(title = "SST x Gulf Stream Index")
```
...
```{r}
# GSI x Landings
ggplot(regression_df, aes(GSI, `Commercial Landings`, color = decade)) +
  geom_point() + facet_wrap(~spectra_area) +
  scale_color_gmri() +
  labs(title = "Gulf Stream Index x Commercial Landings")
```
:::
...
```{r}
#| label: decade-relationships
#| eval: true
#------  Decade. --------

# SST x Decade
ggplot(regression_df, aes(decade, SST)) +
  geom_boxplot()  + facet_wrap(~spectra_area) +
  labs(title = "Decade x SST")
```
...
```{r}
# GSI x Decade
ggplot(regression_df, aes(decade, GSI)) +
  geom_boxplot()  + facet_wrap(~spectra_area) +
  labs(title = "Decade x Gulf Stream Index")
```
...
```{r}
# Landings x Decade
ggplot(regression_df, aes(decade, `Commercial Landings`)) +
  geom_boxplot() + facet_wrap(~spectra_area) +
  labs(title = "Decade x Commercial Landings")
...
## Spectra Slope Breakpoints and Regression Analysis

Following break point exploratory analyses with the EnvCpt package, the Gulf of Maine was the only region from which breakpoints were identified in the best supported model from the 12 candidaate changepoint model structures. Changes in Gulf of Maine's spectrum slope was best modeled by a model allowing breaks in the trend, and with an auto-regressive process on the previous 2 years. From this model, breaks were identified at 1999 and 2007. Georges Bank was best modeled with a single trend, with no breakpoints. Southern New-England showed the most support for a single intercept model, with no breakpoints, with values centered on mu = -1.04 & sd = 0.046. The Mid-Atlantic Bight was best supported by a single trend model, with no breakpoints, and an increasing intercept through the study period.
...
all_parms <- lm(`ISD Exponent` ~ SST + GSI + `Commercial Landings` + zp_small + zp_large + regime, data = gom_df)
...
# Allowing Interactions with the breakpoint periods found from changepoint analysis:
...
# Looks better
# augment(new_bmod, gom_df) %>% 
#   ggplot(aes(year)) +
#   geom_line(aes(y = `ISD Exponent`, color = "Actual"), linewidth = 1) +
#   geom_line(aes(y = .fitted, color = "Regime Change Predicted"), linewidth = 1) +
#   scale_color_gmri() +
#   labs(color = "Color", y = "Spectrum Slope")


# # Yea, better AIC
# AIC(new_bmod); AIC(bmod); AIC(regime_lm)
```
...
# This code fits the autoregressive model from the changepoint analysis
# lag 1 and lag 2 could be accomplished with lag(ISD_exponent, 1) & lag(isd..., 2) if we want to do that also:

gom_dat <- gom_df$`ISD Exponent`
n_gom <- length(gom_dat)

# This is the code from the envcpt()
gom_trendar2.fit = lm(gom_dat[-c(1:2)] ~ c(1:(n_gom - 2)) + gom_dat[2:(n_gom - 1)] + gom_dat[1:(n_gom - 2)])


# Plotting the model from the changepoint algorithm
data.frame(
  "year" = gom_df$year,
  "actual" = gom_dat,
  "fitted" = c(NA,NA, gom_trendar2.fit$fitted.values)) %>% 
  ggplot(aes(x = year)) +
...
gom_df_lag <- mutate(gom_df, 
                 b = `ISD Exponent`,
                 blag1 = dplyr::lag(b, 1),
                 blag2 = dplyr::lag(b, 2)) %>% drop_na(blag2)
...
# Model selection on models with autoregression at 1+2 years
...
augment(best_ar, gom_df_lag) %>%
  ggplot(aes(year)) +
  geom_line(aes(y = `ISD Exponent`, color = "Actual"), linewidth = 1) +
  geom_line(aes(y = .fitted, color = "Regime Change & Autoregression Model Preds"), linewidth = 1) +
  scale_color_gmri() +
  labs(color = "Color", y = "Spectrum Slope", title = "Great Model - OR - Overfit?")
...
# Can we use emmeans to digest this?
emmeans::emmeans(best_ar)
```
...
# fit model with all parameters
all_parms <- lm(`ISD Exponent` ~ SST + GSI + `Commercial Landings` + zp_small + zp_large + regime, data = gb_df)
...
# fit model with all parameters
all_parms <- lm(`ISD Exponent` ~ SST + GSI + `Commercial Landings` + regime, data = sne_df)
