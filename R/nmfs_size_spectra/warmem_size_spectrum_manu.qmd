...
Understanding the impacts of climate change on local and regional scales is a high priority area of study among the international scientific community. The largest determinant of annual climatic timescale variation is the heat exchange between the atmosphere and the ocean @Glantz1991. Major atmospheric and oceanographic climate modes like the El Nino Southern Oscillation (ENSO), the Gulf Stream Index (GSI) or the North Atlantic Oscillation (NAO) can be linked to localized climate impacts via energy transport between the ocean and atmosphere in a linkage referred to as a "teleconnection" @liu2007. The existence of these pathways demands that scientists understand the inter-connected relationship between local weather and large-scale oceanographic regimes. Scientists following climate change through study of Earth's energy imbalance have observed that over 90% of that excess heat from climate change is accumulating in the ocean @meyssignac2019. This fact has re-focused attention on concerns for the future health of marine ecosystems as temperatures, acidification, sea level, and ocean circulation patterns change (@Johnson2020; @findlay2021; @meyer-gutbrod2021; @frederikse2020; @neto2021). Focusing more narrowly on the impacts of rapid temperature change, scientists have predicted that ecological communities will respond in a number of predictable pathways, each to the limit that their constituent species' life histories can support a response. These responses include shifts in geographic distribution annd alterations to their growth and maturation (Bergmanns Rule, @blackburn1999, **CITE early maturation when warm, @atkinson1995** ) . Marine ecosystems are also shaped directly through human interactions like fishing, which has also been shown to impact these single-species characteristics ( CITE: genetic impacts of size-selective gear, allee effects).

Marine ecosystems are complex and are difficult to sample accurately. Much research on the early impacts of climate change on marine systems has focused on individual species or small communities that can be consistently sampled and/or have their interactions explicitly described ( @pinsky2013, @nye2009, @pershing2015 ). An obvious weakness of these approaches is the assumption that all ecological needs provided through inter-species interactions will be equally met under projected climate change conditions. Another lens with which to study the organization of a community is through the study of traits. Size is commonly referenced as the master trait, with well documented relationships with metabolism and bio-energetics @sheldon1972 @brown2000 @white2007 . Marine communities are very-often size-structured, with size correlating to trophic position, making them ideal for study through this lens. Size spectra theory is an ecological framework for studying the relationship between the abundance and size of individuals in a community . This relationship relates how efficiently energy flows from the smallest individuals at the lowest trophic levels to larger individuals occupying higher trophic levels @andersen2006 @hillaert2018 . It is a taxon-agnostic method for detecting changes to ecosystem health and has shown to be sensitive to both large-scale environmental disturbance as well as direct human interaction @shin2005 @kerr2001 . When organized on a log scale, this relationship between abundance and size reveals a linear decline in abundance with increasing body size @white2008 . Highly productive and diverse marine ecosystems are associated with less steep declines in abundance with size, and support larger numbers of higher trophic level individuals. By studying how this relationship changes through time scientists can observe the energy-transfer efficiency of a community as a signal of ecological health and robustness to external changes in the environment @sprules2016 .

Our study is focused on changes in the size distribution finfish community of the Northeast US continental shelf. This is a region that is experiencing rapid increases to temperature tied to changes in the behaviors of the Gulf Stream and Labrador Currents @meyer-gutbrod2021 . This ecosystem has been sampled extensively as part of national fisheries management efforts and has long-term records on the size distribution of the finish community, providing a long-history with which to study changes in size structure. Based on early observational studies and ecological theory we anticipate that individual level responses to a rapidly warming environment will be detectable in the size-structure of the region's ecological community, evidence that ecosystems are experiencing change-of-function as a result of rapid environmental change.
...
plot_abundance_vertical <- function(p_area = "GoM", show_facets = F, ylab = "Abundance (million)"){
  ...
  p <- fgroup_area$weight_bins %>% 
    ...
    complete(Year, hare_group, area_titles, left_lim) %>% 
    ggplot(aes(Year, strat_abund_mill, fill = left_lim)) +
      geom_col(color = "gray30", linewidth = 0.1) +
      facet_grid(hare_group~area_titles, scales = "free") +
...
# Build the pieces
shelf_abunds <- plot_abundance_vertical("Northeast Shelf", show_facets = F)
...
# Build the pieces
shelf_bmass <- plot_bodymass_vertical("Northeast Shelf", show_facets = F)
...
# plot_spectra <- function(area_lab){
#   line_dat <- region_indices %>% 
#     filter(area_titles == area_lab)
#   decade_dat <- indices_decades %>% 
#     filter(area_titles == area_lab)
#   
#   line_dat %>% 
#     ggplot(aes(yr, b, group = area_titles)) +
#     geom_line(
#       aes(group = area_titles), 
#       color = "gray60", 
#       linewidth = 1) +
#     geom_segment(
#       data = decade_dat,
...
#     scale_color_gmri() +
#     facet_wrap(~area_titles, ncol = 1, scales = "free") +
#     labs(x = NULL, y = NULL, caption = NULL, title = NULL, subtitle = NULL)  +
#   theme(plot.margin = margin(t = 0, b = 0))
# }
# 
# # Just the GOM plot needs some added features
# 
# # Just manually do gom...
# line_dat_gom <- region_indices %>% 
#     filter(area_titles == "Gulf of Maine") %>% 
#   mutate(regime_col = ifelse(
#     yr < 1999, gmri_cols("green"),
#     ifelse(yr > 2006, gmri_cols("orange"),
#            gmri_cols("blue"))))
# decade_dat_gom <- indices_decades %>% 
#     filter(area_titles == "Gulf of Maine")
#   
# gom_spectra <- line_dat_gom %>% 
#     ggplot(aes(yr, b, group = area_titles)) +
#     geom_line(
#       aes(group = area_titles,
#           color = I(regime_col)), 
...
# 
# # Then do the others
# shelf_spectra <- plot_spectra(area_lab = "Northeast Shelf")
# gb_spectra <- plot_spectra(area_lab = "Georges Bank")
# mab_spectra <- plot_spectra(area_lab = "Mid-Atlantic Bight")
# sne_spectra <- plot_spectra(area_lab = "Southern New England")
# 
# shelf_spectra /
#   gom_spectra /
#   (gb_spectra +
#   labs(y = "Size Spectra Slope (b)")) /
#   sne_spectra /
#   mab_spectra +
#   labs(x = "Year")
...
spectra_heatmap_data <- catch_size_bins %>% 
  ...
  map_dfr(function(x){
    x %>% 
     ...
     ungroup() %>% 
      mutate(
        single_year_z = (single_year_total_abund - all_year_avg) / all_year_sd)
  }, .id = "area_titles") %>% 
  mutate(area_titles = factor(area_titles, levels = area_levels_long))
...
spectra_heatmaps <-
  ...
  labs(y = "Body Weight (g)",
       fill = "Size-Specific Relative Abundance (z)")
...
# # Question:
# # Should I normalize the abundances by bin-width first
# # Then show how abundances shifted on that all-sizes scale?
# maybe
# 
# Yea probably, jk... magnitudes are way off
# Maybe within a year?
# This code and its output should be

# norm_test <- catch_size_bins %>% 
#   mutate(area_titles = "Northeast Shelf") %>% 
#   bind_rows(catch_size_bins) %>% 
#   split(.$area_titles) %>% 
#   map_dfr(function(x){
...
#         single_year_total_abund = sum(strat_total_abund_s, na.rm = T),
#         .groups = "drop") %>%
#       # Do we normalize first and then ignore the bins?
#       mutate(
#         single_year_norm = single_year_total_abund / bin_width
#       ) %>%
#       # Now I'm confused what to average and standardize against...
#       # I think I want the normalized abundance
#       # to be scaled against the average normalized abundance and variance across all years
#       group_by(area_titles) %>%
#       mutate(
#         all_size_avg = mean(single_year_norm, na.rm = T),
#         all_size_sd = sd(single_year_norm, na.rm = T)) %>%
#       ungroup() %>%
#       mutate(single_year_z = (single_year_norm - all_size_avg) / all_size_sd)
#   }, .id = "area_titles") %>% 
#   mutate(area_titles = factor(area_titles, levels = area_levels_long))
# 
# 
...
#     barheight = unit(8, "cm"))) +
#   labs(y = "Body Weight (g)",
#        fill = "Change in All-Size Normalized Abundance (z)")
...
```{r}
#| eval: false
#| label: surfaces check


# Plot the surface using the actual abundances
####  Surface Plots
spectra_surface <- function(log2_totals, plot_title){
  
  # Pull data for matrix
  dat <- dplyr::select(log2_totals, left_lim, Year, norm_strat_abund)
  
  # Make matrix to plot
  spectrum_mat <- as.matrix(xtabs(norm_strat_abund~left_lim + Year, data = dat))
  
  
  # Make a flat surface of -1 slope?
  ...
  add_surface() %>%
  plotly::layout(
    title = str_c(plot_title, " - Normalized Size Spectrum"),
    scene = list(
      xaxis = list(title = "Year", autorange = "reversed"),
      yaxis = list(title = "Log2( BodyMass (g) )"),
      zaxis = list(title = "Log2( Normalized Abundance) "),
...
# Plot it using the normalized abundances
# Get annual totals within each area
areas_aggregated <- catch_size_bins %>% 
  split(.$area_titles) %>% 
  map(function(area_data){
    area_data %>% 
      split(.$Year) %>% 
      map_dfr(function(yr_data){
        aggregate_log2_bins(yr_data) %>%
          mutate(
            log2_bins = as.numeric(log2_bins),
            norm_strat_abund = log2(norm_strat_abund),
...
# Gulf of Maine
spectra_surface(
  log2_totals = filter(areas_aggregated$`Gulf of Maine`, Year >= 1970), 
  plot_title = "Gulf of Maine")


# Everywhere
shelf_bin_agg <- catch_size_bins %>% 
  split(.$Year) %>% 
      map_dfr(function(yr_data){
        aggregate_log2_bins(yr_data) %>%
          mutate(
...
spectra_surface(
  log2_totals = filter(shelf_bin_agg, Year >= 2000),
  plot_title = "Northeast Shelf")
...
# At this point it is confusing how to read how
# the regimes impact the fixed effects
...
# Pull the regime parts out
# Any terms that are involved in interactions
regime_coefs <- top_coef #%>% 
  # split(.$`Model ID`) %>% 
  # # everything here down will need to be in map()
  # pluck(1) %>% 
  # filter(str_detect(term, ":|regime|landings|large"))
...
# Isolate the intercept adjustment from the regimes
regime_adj <- p1_predictors %>% 
  filter(
    str_detect(term, "regime")) %>% 
  mutate(
...
# Pull out the interactions and label the fixed effect and the regime
...
primary_terms %>% 
  left_join(bind_rows(regime_adj, regime_interax)) %>% 
