...
# fgroup_area <- get_group_summaries(data_in = nefsc_size_bins, Year, survey_area, hare_group)
# fgroup_all <- get_group_summaries(data_in = nefsc_size_bins, Year, hare_group)
# fgroup_all <- fgroup_all %>% map(~mutate(.x, survey_area = "Northeast Shelf, Full"))


# Actual loog10 bins
# Do some grouping by year survey area and functional group
fgroup_area <- l10_bin_summaries(data_in = nefsc_size_bins, Year, survey_area, hare_group)
fgroup_all  <- l10_bin_summaries(data_in = nefsc_size_bins, Year, hare_group)
fgroup_all  <- fgroup_all %>% map(~mutate(.x, survey_area = "Northeast Shelf, Full"))
...
####  Old Way, big mess
# All messed up, check github


# # 1. Set the two periods up
# driver_periods <- all_drivers %>% 
#   mutate(period = ifelse(year <= 2005, "regime_1", "regime_2")) %>% 
...
# 
# 
# # Need to Reshape and Scale Before anything else happens
# driver_matrix <-  driver_periods %>% 
#     mutate(id = str_replace_all(str_c(area, "_", var), " ", "_")) %>% 
#     select(-c(area, var)) %>% 
#     pivot_wider(names_from = id, values_from = value) 
# 
# # Split the periods into their own lists
# driver_period_l <- driver_matrix %>%
#     split(.$period) %>% 
#     map(~ .x %>% drop_na() %>% column_to_rownames(var = "year"))
#     
...
d_test  <- driver_period_l$regime_2
...
period_correlations <- map(driver_period_l, function(drivers){
  ...
  # Step 2. 
  # Make correlation matrix a dataframe for GGplot
  gg_rho <- as.data.frame(rho) %>%
    rownames_to_column(var = "xvar") %>%
    pivot_longer(cols = -xvar, names_to = "yvar", values_to = "r2")
...
fgroup_area$weight_bins <- fgroup_area$weight_bins %>% 
  mutate(strat_abund_mill = wtbin_strat_abund / 1e6,
         ...
         survey_area = factor(survey_area,
                              levels = c("Northeast Shelf, Full", "GoM", "GB", "SNE", "MAB")))
...
period_correlations$regime_1$only_driver_response %>% 
  ...
  labs(x = "Hypothesized Driver", y = "Bodymass Spectra Coefficient",
       fill = "Correlation Coefficient",
       title = "Correlation of Size Spectra to Hypothesized Drivers | Regime 1",
       subtitle = "Correlation coefficients shown display same-year correlations.")
 ...
 * Top-down and bottom up influences on both carrying capacity (intercept) and transfer efficiency (slope)
 ...
 Literature suggests:
  ...
  - Slope (a measure of energy transfer efficiency and static biomass distribution) has been shown to be sensitive to the physical removal of larger individuals.
  
  
Temperature Mechanisms:
  - Temperature's impact on growth via genetic plasticity impacts both the available biomass at the primary producer level, as well as the Linf of larger species.
