...
ss_idx <- region_indices %>% 
  select(
    year = Year, 
    survey_area, 
    spectra_slope = l10_slope_strat, 
    spectra_int = l10_int_strat,
    isd_slope = b) %>% 
  pivot_longer(
  ...
  gg_rho_tidy <- left_join(gg_rho, gg_sig, by = c("xvar", "yvar")) %>% 
    mutate(
      ...
      x_short = case_when(
        ...
        str_detect(xvar, "slope") ~ "Spectra Slope",
        str_detect(xvar, "int") ~ "Spectra Intercept"),
      x_short = factor(x_short, 
                       levels = c("Spectra Intercept",
                                  "Spectra Slope",
                                  "ISD Exponent")))
