...
The impact of external factors on the changes in size spectra was correlated against several hypothesized driving forces related to both environmental regimes and anthropogenic disturbances. Potential large-scale environmental drivers include regional SST anomalies & the larger-scale impact of the Gulf Stream Index (GSI). Fishing pressure represents the primary top-down anthropogenic driver in the region, and was investigated using the aggregate regional commercial landings data. 
...
sizes_wide <- functional_group_size %>% 
  select(year = Year, 
         survey_area, 
         hare_group, #avg_length = mean_len_cm, 
         avg_weight = mean_wt_kg) %>% 
  pivot_longer(names_to = "var", 
               values_to = "value", 
               cols = c(avg_weight)# ,avg_length)
               ) %>% 
...
region_indices %>% 
  ...
  facet_wrap(~survey_area, ncol = 1) +
  labs(x = "Normalized Abundance Spectra Slope", y = "Individual Size Distribution Exponent (b)", title = "Method Correlation")
```
:::
