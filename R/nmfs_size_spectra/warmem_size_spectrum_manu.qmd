...
ggplot() +
  ...
  labs(title = "Groundfish Survey Regions")
```
...
# Reshape Wide
# Scale the columns independently using the variance in all years
...
# Drivers scaled is a matrix of all the different timeseries
...
driver_ccf_prep <- drivers_scaled %>% 
  ...
  mutate(
    ...
    community_area = factor(community_area, levels = c("All", "Gulf of Maine", "Georges Bank", "Southern New England", "Mid-Atlantic Bight")),
...
driver_ccf_prep <- driver_ccf_prep %>% 
  filter(driver_area == community_area | driver_area == "All") %>% 
...
strat_abundance_plot <- fgroup_area$weight_bins %>% 
  ...
  labs(y = "Abundance (millions)",  
       caption = "Estimated Abundance from\nArea-Stratified Catch Rates",
         x = "",
...
# int_timeline <- ggplot(region_indices, aes(yr, log2_int_strat, group = survey_area)) +
#   geom_line(data = select(region_indices, -survey_area),
#             aes(group = area_copy),
#             alpha = 0.2, linewidth = 0.5) +
#   geom_line(aes(group = survey_area), linewidth = 1) +
...
period_correlations <- map(driver_period_l, function(drivers){
  ...
  # in an orderly way
  # xvar and yvar are the order that they go into the corrrelation estimation
  gg_rho_tidy <- left_join(gg_rho, gg_sig, by = c("xvar", "yvar")) %>% 
    mutate(
      # A. Slope and intercept are the only size spectra features
      # and also
      response_var = ifelse(str_detect(xvar, "slope"), T, F),
      
      # B.These are the drivers, use string detect to pull them out
      driver_var = ifelse(str_detect(yvar, "landings|index|anom"), T, F),
      
      # C. Label Top down and Bottom Up Drivers
      driver_type = case_when(
        str_detect(yvar, "landings") ~ "Top-Down",
        str_detect(yvar, "stream|anom") ~ "Bottom-Up"),
      
      # D. These are the areas associated with the Spectra Features
      xvar_area = case_when(
      ...
      # F. These are the areas of the drivers
      yvar_area = case_when(
      ...
      yvar_area = factor(yvar_area, levels = c(
        "All", 
        "Gulf of Maine", 
        "Georges Bank", 
        "Southern New England", 
      ...
      x_short = case_when(
        str_detect(xvar, "isd") ~ "ISD Exponent",
...
garfo_map <- ggplot(stat_zones) +
  ...
  scale_fill_gmri() +
  theme_bw() +
  map_theme(legend.position = c(0.7, 0.25),
            legend.background = element_rect(fill = "white")) +
  guides(fill = guide_legend(title = "", nrow = 2)) +
