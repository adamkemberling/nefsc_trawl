...
df <- df %>% 
  ...
  mutate(
    ...
    ersst_z = as.numeric(base::scale(ersst)),
...
df  %>% 
  ...
  scale_color_gmri() +
  labs(title = "Body Size Declining - Early",
...
df  %>% 
  ...
  geom_smooth(method = "lm", linewidth = 1, se = F) +
  scale_color_gmri() +
  labs(title = "Spectra Slope Steepening - Except MAB",
...
df  %>% 
  ...
  geom_smooth(method = "lm", linewidth = 1, se = F) +
  scale_color_gmri() +
  labs(title = "SSTs Rising Area-Wide",
...
df  %>% 
  ...
  geom_smooth(method = "lm", linewidth = 1, se = F) +
  scale_color_gmri() +
  labs(title = "Bottom Temperatures Rising - Except MAB",
...
df %>% 
  ...
  labs(
    title = "Regional Landings Declining - Bimodal in MAB",
...
df %>% 
  ...
  scale_x_continuous(limits = c(1970, 2020)) +
  labs(
    title = "Regional Landings Declining - Peak Overall in 90's-2000's",
    subtitle = "Mid-Atlantic Exception, primarily forage fish fishery (menhaden)",
    x = "Year",
    color = "Region", fill = "Region")


# Trying to keep landings: binned/categorical label
...
Bart raised that his landings looked different, can verify that we didn't mess them up in data prep.
...
ggplot(df, aes(ersst_z, landings)) + 
  geom_point() + 
  geom_smooth(method = "lm") +
  facet_wrap(~survey_area) +
...
# Complete model
full_mod <- lme4::lmer(
  b ~ land_5  * region * ersst_z + (1 | year), 
  data = drop_na(df, land_5, ersst_z))
...
sst_only_mod <- lme4::lmer(
  b ~ region * ersst_z + (1 | year), 
  data = drop_na(df, land_5, ersst_z))
...
plot(check_collinearity(slope_sst))
plot(check_collinearity())
plot(check_collinearity(full_mod))
...
anova(slope_sst, full_mod)
anova(landings_only_mod, full_mod)
anova(sst_only_mod, full_mod)


# Based on chi-square test
anova(no_sst_mod, full_mod, test = "LRT")
...
# Do AICc
MuMIn::AICc(
  full_mod,slope_landings, slope_sst, sst_only_mod, landings_only_mod) %>%  
...
```{r}
# SST Anomaliezs
plot(ggpredict(full_mod, ~ersst_z+region), add.data = T, jitter = 0.1)
...
# Complete model
full_mod <- lme4::lmer(
  med_wt ~ land_5  * region * ersst_z + (1 | year), 
  data = drop_na(df, land_5, ersst_z))
...
plot(check_collinearity(slope_landings))
plot(check_collinearity(slope_sst))
plot(check_collinearity(sst_only_mod))
...
Could also do year within region nesting...
...
```{r}
# new model
full_mod <- lme4::lmer(med_wt ~ landings * sst_anom + (1 | survey_area), 
                      data = drop_na(df, landings, sst_anom))

# # Autocorrelative Error - Performs badly here
# Ben bolker to thatnk, of course:
# https://bbolker.github.io/mixedmodels-misc/notes/corr_braindump.html
full_mod <- nlme::lme(med_wt ~ landings  * landings + sst_anom,
                     random = ~ 1 |  survey_area,
                     data = drop_na(df, landings, sst_anom),
                     correlation = corAR1(), 
                     method = "ML")
...
ggplot(df, 
    ...
    scale_color_gmri() +
      geom_line(
        data = cbind(drop_na(df, landings, sst_anom), 
                     pred = predict(full_mod)), 
        aes(y = pred), size = 1) +  # adding predicted line from mixed model 
      theme(legend.position = "none",
            panel.spacing = unit(2, "lines"))  # adding space between panels
...
```{r}
# multicollinearity check
col_check <- check_collinearity(full_mod)
plot(col_check)
```
...
```{r}
# SST Anomaliezs
plot(ggpredict(full_mod, ~sst_anom), add.data = T, jitter = 0.1)
...
```{r}
plot(ggpredict(full_mod, ~landings), add.data = T, jitter = 0.1)
```
