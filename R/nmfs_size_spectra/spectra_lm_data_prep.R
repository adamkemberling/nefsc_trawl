...
# # Export for Bart
# write_csv(region_indices, here::here("data/multispecies_spectra_results.csv"))
...
drivers_scaled <- read_csv(here::here("data/env_drivers_scaled.csv"))

# Or the raw versions
# This is the unscaled predictor dataset
unscaled_drivers <- read_csv(here::here("data/unscaled_spectra_predictor_df.csv"))
...
# unscaled_prep <- unscaled_drivers %>% 
#   mutate(
#     metric = case_when(
#       str_detect(metric, "SST") ~ "sst",
#       str_detect(metric, "Landings") ~ "landings",
#       str_detect(metric, "GSI") ~ "gsi",
#       TRUE ~ metric)) 
# 
# # Pull out gsi
# gsi_raw <- unscaled_prep %>% 
#   filter(metric == "gsi") %>% 
#   distinct(year, metric, value) %>% 
#   select(year, gsi = value)
# 
# # Widen and join back gsi
# unscaled_wide <- unscaled_prep %>% 
#   filter(metric != "gsi", metric != "Spectra Slope") %>% 
#   select(year, survey_area = spectra_area, metric, value) %>% 
#   # # DUPE check
#   # dplyr::group_by(year, survey_area, metric) %>%
#   # dplyr::summarise(n = dplyr::n(), .groups = "drop") %>%
#   # dplyr::filter(n > 1L) 
#   pivot_wider(names_from = metric, values_from = value) %>% 
#   left_join(gsi_raw)
# write_csv(unscaled_wide, here::here("data/unscaled_spectra_predictor_dataset_wide.csv"))


# # Write out sst in raw form
# # Load the regional temperatures from {targets}
# withr::with_dir(rprojroot::find_root('_targets.R'), 
#                 tar_load(regional_oisst))
# regional_oisst %>% left_join(area_df) %>% 
#   select(year = yr, survey_area= area_titles, sst, sst_anom) %>% 
#   write_csv(here::here("data/unscaled_regional_sst.csv"))
...
# sst & gsi highly correlated, we want* to keep sst
...
# Do for ERSST?
plot_corr("ersst_anom", "gsi")
plot_corr("ersst_anom", "year")
plot_corr("ersst_anom", "landings")
plot_corr("ersst_anom", "zp_large")


# Again, colinear with gsi
...
# Remaining conflicts: colinearity with years
plot_corr("sst", "year")
plot_corr("landings", "year")
plot_corr("zp_large", "year")
plot_corr("gsi", "year")
...
####  Export  ####
write_csv(model_data, here::here("data/size_and_spectra_model_data.csv"))
