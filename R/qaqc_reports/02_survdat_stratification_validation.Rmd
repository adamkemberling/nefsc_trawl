...
# ####  Load Feb 23rd starting data, 2021
# load(paste0(res_path, "NMFS_trawl/NEFSC_BTS_02242021.RData"))
# trawldat <- survey$survdat %>% clean_names() %>% rename(est_year = year)
# rm(survey)
...
# survdat_20 <- survdat_prep(survdat = trawldat)
...
test_species <- unique(weights_21$comname)[2]
...
**NOTE: 1/4/2021** Data from here through the bottom is restricted to the albatross survey, before any issues with `numlen_adj` come into play.
...
weights_all <- survdat_prep_nodrop(survdat_source = "most recent") %>% 
  ...
  # Validation Step:
  # Change total LW weight to ignore numlen_adjusted conversion
  # mutate(sum_weight_kg = ind_weight_kg * numlen) %>% 
  add_area_stratification() 

# During troubleshooting we filtered down to just use albatross data
# weights_all <- weights_all %>% filter(svvessel == "AL")
