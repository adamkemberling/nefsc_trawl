...
# need to normalize them by bin width so that they are comparable.
...
bin_summs <- nefsc_stratified %>% 
  ...
  mutate(l_norm     = n_group / len_range,
         ...
         `min length (cm)`  = as.character(round(len_range, 2)),
         `min weight (kg)`  = as.character(round(wt_range, 2)),
         `log10 min weight` = as.character(round(wt10_range, 2)))


# Use factor level formatting to put meaningful labels on the bins
sort_numeric_chars <- function(num_as_text){
  as.character(sort(as.numeric(unique(num_as_text))))
}

len_levels  <- sort_numeric_chars(bin_summs$`min length (cm)`)
wt_levels   <- sort_numeric_chars(bin_summs$`min weight (kg)`)
wt10_levels <- sort_numeric_chars(bin_summs$`log10 min weight`)


# apply factor levels
bin_summs <- bin_summs %>% 
  mutate(
    `min length (cm)` = factor(`min length (cm)`, levels = len_levels),
    `min weight (kg)` = factor(`min weight (kg)`, levels = wt_levels),
    `log10 min weight` = factor(`log10 min weight`, levels = wt10_levels)
  )
...
# log() length bins
l1 <- ggplot(bin_summs, aes(x = `min length (cm)`, y = n_group, fill = svvessel)) +
    geom_col(show.legend = F) +
    scale_y_continuous(labels = scales::comma_format()) +
    facet_wrap(~svvessel) +
    scale_fill_gmri() +
    labs(x = "Minimum Length (cm)", y = "Abundance") +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5))

l2 <- ggplot(bin_summs, aes(`min length (cm)`, l_norm, fill = svvessel)) +
    geom_col(show.legend = F) +
    scale_y_continuous(labels = scales::comma_format()) +
    facet_wrap(~svvessel) +
    scale_fill_gmri() +
    labs(x = "Minimum Length (cm)", 
         y = "Normalized Abundance", 
         caption = "Bin numbers increase at increments of 0.5 in log(length).") +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
...
# log() weight bins
wt1 <- ggplot(bin_summs, aes(`min weight (kg)`, n_group, fill = svvessel)) +
    geom_col(show.legend = F) +
    scale_y_continuous(labels = scales::comma_format()) +
    facet_wrap(~svvessel) +
    scale_fill_gmri() +
    labs(x = "Minimum Weight (kg)", y = "Abundance") +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5))

wt2 <- ggplot(bin_summs, aes(`min weight (kg)`, wtbin_norm, fill = svvessel)) +
    geom_col(show.legend = F) +
    scale_y_continuous(labels = scales::comma_format()) +
    facet_wrap(~svvessel) +
    scale_fill_gmri() +
    labs(x =  "Minimum Weight (kg)", 
         y = "Normalized Abundance", 
         caption = "Bin numbers increase at increments of 2 in log(weight).") +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
...
#l10 weight bins
wt10_1 <- ggplot(bin_summs, aes(`log10 min weight`, n_group, fill = svvessel)) +
    geom_col(show.legend = F) +
    scale_y_continuous(labels = scales::comma_format()) +
    facet_wrap(~svvessel) +
    scale_fill_gmri() +
...
wt10_2 <- ggplot(bin_summs, aes(`log10 min weight`, wt10_norm, fill = svvessel)) +
    geom_col(show.legend = F) +
    scale_y_continuous(labels = scales::comma_format()) +
    facet_wrap(~svvessel) +
    scale_fill_gmri() +
...
# Plotting l10 weight bins

# functions to reduce code repetition
plot_l10bins <- function(x){
  x %>%
  ggplot(aes(`log10 min weight`, n_group, fill = svvessel)) +
        geom_col(show.legend = F) +
        scale_y_continuous(labels = scales::comma_format()) +
        facet_grid(est_year ~ svvessel, scales = "free") +
        scale_fill_gmri() +
        labs(x = "Min Weight (kg)", y = "Abundance") +
        theme(strip.text.y = element_text(angle = 0),
              axis.text.x = element_text(angle = 90, vjust = 0.5))
}

# Normalized abundances
plot_l10bins_norm <- function(x){
  x %>%
  ggplot(aes(`log10 min weight`, wt10_norm, fill = svvessel)) +
        geom_col(show.legend = F) +
        scale_y_continuous(labels = scales::comma_format()) +
        facet_grid(est_year ~ svvessel, scales = "free") +
        scale_fill_gmri() +
        labs(x = "Min Weight (kg)", y = "Normalized Abundance") +
        theme(strip.text.y = element_text(angle = 0),
              axis.text.x = element_text(angle = 90, vjust = 0.5))
}
