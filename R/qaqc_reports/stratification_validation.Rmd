...
For example values `r perc_cutoff`% heavier or lighter.
...
Certain coefficients for some species are not well aligned with the biomass recorded in the survey. These contribute to the big differences in stratified abundances/biomasses. If we set a reasonable cutoff threshold of `r perc_cutoff`%, these are what the timelines now look like and how many species get dropped from each source.
...
Species from the Wigley paper that did not meet the standards of the `r perc_cutoff`% cutoff threshold were:
...
The fishbase species that pass the `r perc_cutoff`% cutoff are the following `r length(fishbase_okay)` species:
...
Across all years the amount of biomass included in the `r length(wigley_okay)` species is **`r biom_perc`%** of the biomass caught in the survey.
