...
ann_summ_bio <- weights_21 %>% 
  distinct(est_year, id, comname, catchsex, biomass, biom_per_lclass, expanded_biom_s) %>% 
  group_by(est_year) %>% 
  summarise(`Bulk Biomass` = sum(biomass),
...
ann_summ_lw <- weights_21 %>% 
  distinct(est_year, id, comname, length, catchsex, biomass, 
...
seas_summ_bio <-  weights_21  %>% 
  distinct(est_year, season, id, comname, catchsex, biomass, biom_per_lclass, expanded_biom_s) %>% 
...
```{r}
weights_cutoff <- weights_21 %>% filter(comname %in% wigley_okay)
