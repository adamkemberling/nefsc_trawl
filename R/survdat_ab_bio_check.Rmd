...
After that repeated records for abundance and biomass are dropped. These repeats exist to acommodate the length and number at length details. We don't need them for this so they are dropped here.
...
## Visualizing the three data sources Abundance/Biomass {.tabset}
...
timeseries_plots <- map(even_groups, function(species_subset){
  ...
  abundance_plot <- ggplot(species_subset_dat, aes(est_year)) +
    geom_line(aes(y = abund_19, color = "Survdat Nye Allseason 2019"), group = 1) +
    geom_line(aes(y = abund_20, color = "Survdat Nye - August 2020"), group = 1) +
    geom_line(aes(y = abund_21, color = "Newest Survdat"), group = 1) +
    scale_y_continuous(labels = scales::comma_format()) +
    scale_color_gmri() +
    facet_wrap(~comname, scales = "free") +
    labs(x  = "", y = "Annual Abundance", color = "Survdat Retrieval") +
    theme(legend.position = "none")
  
  
  biomass_plot <- ggplot(species_subset_dat, aes(est_year)) +
    geom_line(aes(y = biom_19, color = "Survdat Nye Allseason 2019"), group = 1) +
    geom_line(aes(y = biom_20, color = "Survdat Nye - August 2020"), group = 1) +
    geom_line(aes(y = biom_21, color = "Newest Survdat"), group = 1) +
    scale_y_continuous(labels = scales::comma_format()) +
    scale_color_gmri() +
    facet_wrap(~comname, scales = "free") +
    labs(x  = "", y = "Annual Biomass", color = "Survdat Retrieval") +
