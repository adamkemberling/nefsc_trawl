...
Length and weight at first maturity for each species has been taken from [NOAA fisheries](https://www.fisheries.noaa.gov/species-directory) and from fishbase when unavailable.

```{r}

size_at_maturity <- tribble(
  ~"Common Name",          ~"Length at Maturity",      ~"Weight at Maturity",
  "american plaice",
  "atlantic cod",
  "atlantic herring",
  "atlantic mackerel",
...
mean_saa_plots <- function(comname_x, test_var = avg_len, var_label = "Average Length (cm)"){
  ...
  ggplot(aes(year, {{test_var}})) +
    ...
    geom_segment(
      ...
      labs(title = comname_x, y = var_label, x = "Year")
...
decadal_growth_lines <- function(test){
  # Data to get averages
  species_dat <- filter(age_truncated, comname == test)
  
  # Get the averages for each year, give labels for display groups
  age_lines <- species_dat %>% 
      filter(yearclass >= 1970) %>% 
      group_by(comname, year = yearclass,  age) %>% 
    summarise(
      avg_len = mean(length_cm, na.rm =T),
      avg_wt = mean(indwt, na.rm =T),
      avg_condition = mean(fK, na.rm =T),
  ...
  decade_lines <- species_dat %>%
    filter(yearclass>= 1970) %>% 
    mutate(decade = floor_decade(yearclass)) %>% 
    group_by(comname, decade,  age) %>% 
    summarise(
      avg_len = mean(length_cm, na.rm =T),
      avg_wt = mean(indwt, na.rm =T),
      avg_condition = mean(fK, na.rm =T),
  ...
  len_plot <- ggplot(age_lines, aes(age, avg_len)) +
    geom_line(aes(group = year, color = decade), alpha = 0.2, linewidth = 0.5) +
    scale_color_gmri() +
    geom_line(data = decade_lines, aes(group = decade, color = decade), linewidth = 1) +
    geom_dl(data = decade_lines, aes(group = decade, color = decade, label = decade), method="last.points") +
    scale_x_discrete(expand = expansion(add = c(0,1))) +
    labs(title = test, y = "Average Length (cm)", x = "Age", color = "Decade of Birth")
  ...
  wt_plot <- ggplot(age_lines, aes(age, avg_wt)) +
    ...
    scale_color_gmri() +
    geom_line(data = decade_lines, aes(group = decade, color = decade), linewidth = 1) +
    geom_dl(data = decade_lines, aes(group = decade, color = decade, label = decade, fontface = "bold"), method="last.points") +
    scale_x_discrete(expand = expansion(add = c(0,1))) +
    labs(y = "Average Weight (kg)", x = "Age", color = "Decade of Birth")
  
  both_p <- (len_plot / wt_plot) + plot_layout(guides = "collect")
  return(both_p)
}
