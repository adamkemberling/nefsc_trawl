...
Kruskall / Bartletts Variance - Test of Equal Variance

Welch's ANOVA

```{r}
# Run one length at age thing:
...
period_means <- map(period_groups, function(period_dat){
...
####. WORKING HERE. #####
# Look at curves and pick ranges that work
...
length_age_tests <- map_dfr(setNames(vb_species, vb_species), possibly(function(comname_x){
...
length_age_tests %>% 
  ...
  gt(rowname_col = "age") %>% 
  sub_missing(missing_text = "") %>% 
  tab_header(title = "Length at Age - Two-Sample Test", subtitle = "Comparison of 1970-2009 & 2009-2019 Data")

```



# Weight at Age Anovas

```{r}
#| eval: false

# Run some weight at age


wt_at_age_anovas <- map(vb_species, function(comname_x){
  ...
  # Grab data
  compare_1 <- bind_rows(
    list("regime1970-2009" = filter(period_groups$`1970-2009`, comname == comname_x),
         "regime2010-2019" = filter(period_groups$`2010-2019`, comname == comname_x),
    .id = "comparison_period"))
  
  
  
  
  # Perform Anova for each age
  aov_1     <- aov(indwt ~ comparison_period, data = compare_1)
  
  
  # Check Variance Assumption
  bart_1 <- bartlett.test(indwt ~ comparison_period, data = compare_1)
  
  # Perform the Welchs
  
  
  
  # # Period Comparison 2: 2000-2009 vs. 2010-2019
  # compare_2 <- bind_rows(
  #   list("regime1970-2009" = filter(period_groups$`1970-2009`, comname == comname_x),
  #        "regime2010-2019" = filter(period_groups$`2010-2019`, comname == comname_x),
  #   .id = "comparison_period"))
...
| Species | Age | Mean 1970-2009 | mean 2010-2019 | pval | mean 2000-2009 | pval |
|---------|-----|----------------|----------------|------|----------------|------|
|         |     |                |                |      |                |      |
|         |     |                |                |      |                |      |
