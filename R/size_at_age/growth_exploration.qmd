...
# #winloss_results_1
# 
# # What data each row provides
# df_ex <- winloss_results_1[1,]
# 
# # Function to make a ggplot from the win/loss list
...
#       vals == 0.5 ~ gmri_cols("light gray"), 
#       vals == 1 ~ gmri_cols("blue"), 
#       vals == 0 ~ gmri_cols("orange")))
#   
#   # Build the plot:
...
#     scale_x_continuous(limits = c(1, length(vals)),
#                        breaks = seq(1, length(vals), 1)) +
#     scale_y_continuous(limits = c(-0.5, 1.5))
# }
# 
# # # Example of it in use
# # ex_plot <- my_ggplot(df_ex$wins |> unlist())
# # ex_plot <- my_ggplot(df_ex$wins |> unlist())
#  
# 
# 
# 
# 
# # Then how it is used for gt()
# df_ex |>
#   gt() |>
#   text_transform(
#     locations = cells_body(wins),
#     fn = function(x){
#       ggplot_image(ex_plot, aspect_ratio = 1.5)
#       # ggplot_image(
#       #   my_ggplot(x |> unlist()), 
#       #   aspect_ratio = 1.5)
#     }) |>
#   cols_label(wins = "Growth Change") %>%
#   fmt_markdown(comname)
# 
# 
# # # ^ that works for one example, but how does one include that programmatically?
# # # No
# # winloss_results_1 %>% 
# #   mutate(row = n()) %>% 
# #   split(.$row) %>% 
# #   map(function(.x){
# #     my_plot <- my_ggplot(.x$wins %>% unlist())
# #     })
# 
# 
# # Whole thing?
...
#   gt(rowname_col = "comname") %>%
#   text_transform(
#     locations = cells_body(`Individual Length`),
#     fn = function(x){
#       ggplot_image(ex_plot, aspect_ratio = 3)
#     }) |> 
#   text_transform(
#     locations = cells_body(`Individual Weight`),
#     fn = function(x){
#       ggplot_image(ex_plot, aspect_ratio = 3)
#     }) |>
#   tab_spanner(
...
#     columns = c(`Individual Length`, `Individual Weight`)) %>% 
#   tab_footnote(
#     footnote = md('Colored arrows indicate significant changes in an ageclass characteristic moving from the period of 1970-2009 to individuals from 2010-2019. Downward facing arrows in <a style="color:#EA4F12">orange indicate significant declines in length or weight.</a> Upward facing arrows colored in <a style="color:#00608A">blue indicate significant increases in length or weight.</a> And bars in <a style="color:#B3B3B3">gray</a> indicate no difference in ageclass characteristics.</a>' ),
#     locations = cells_column_labels(
#       columns = c(`Individual Length`, `Individual Weight`)
#   )) %>% 
#   gt::cols_label(
  ...
  text_transform(
    ...
    fn = function(x) {
      lapply(
        ...
        function(x){ggplot_image(plot_fn_arrow(x), aspect_ratio = aspect_r)}
      )
    } # Close fn
  )
