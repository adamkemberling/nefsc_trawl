...
temp_avgs <- median_df %>% 
  ...
  summarise(
    biom_avg = mean(biom_mu),
    biom_med = mean(biom_median)
  )
```
...
Across all species the average temperature (by biomass) has increased from `r lat_avgs[lat_avgs$temp_regime == "Early Regime", 2][[1]]` to `r lat_avgs[lat_avgs$temp_regime == "Warm Regime", 2][[1]]` for a change in `r diff(lat_avgs$biom_avg)` ° Latitude. 
...
avg_depths <- depth_df %>% 
  group_by(temp_regime) %>% 
  summarise(biom_avg = mean(biom_mu),
            biom_med = mean(biom_median),
            .groups = "drop")
```
