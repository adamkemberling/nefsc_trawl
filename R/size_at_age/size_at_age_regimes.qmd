...
# landmass shapes
us_poly <- ne_download(scale = 'large', type = "states",
                       category = "cultural",
                       returnclass = "sf")
...
temp_details <- function(temp_dataset){
    ...
    data_late <- annual_avgs %>% filter(year %in% c(2010:2021))
...
# NE shelf heatwaves
shelf_hw <- pull_heatwave_events(
  temperature_timeseries = shelf_oisst, 
  date_col = "time", 
...
gom_hw <- pull_heatwave_events(
  temperature_timeseries = gom_oisst, 
  date_col = "time", 
  temp_col = "area_wtd_sst", 
  threshold = 90, 
  detrend = FALSE) %>% 
...
heatwave_heatmap_plot <- function(hw_dat, temp_units = "C", start_yr = 1981, end_yr = 2021){
  ...
  heatwave_heatmap <- ggplot(hw_dat, 
    ...
    guides("fill" = guide_colorbar(title = "Sea Surface Temperature Anomaly", 
                                   ...
                                   frame.colour = "black", 
                                   ticks.colour = "black")) +  
    theme(legend.title = element_text(angle = 90), legend.position = "right") +
...
```{r}
shelf_hw <- heatwave_heatmap_plot(hw_dat = shelf_hw, temp_units = "C", start_yr = 1982, end_yr = 2021) + 
  theme_bw() +
  theme(legend.title = element_text(angle = 90)) +
  labs(caption = NULL, title = "Northeast Shelf")
...
```{r}
gom_hw <- heatwave_heatmap_plot(hw_dat = gom_hw, temp_units = "C", start_yr = 1982, end_yr = 2021) + 
  theme_bw() +
  theme(legend.title = element_text(angle = 90)) +
  labs(caption = NULL, title = "Gulf of Maine Ecological Production Unit")

gom_hw
```


### Rates and Regime Averages

```{r}


sst_summary_table <- area_summaries %>% 
  map_dfr(~data.frame(
      "rate_1982_2021_C" = .x$annual_rate,
      "rate_1982_2021_F" = .x$rate_f,
      "mean_temp_1982_2009_C" = .x$mean_early,
      "mean_anom_1982_2009_C" = .x$mean_anom_early,
      "mean_temp_2010_2021_C" = .x$mean_late,
      "mean_anom_2010_2021_C" = .x$mean_anom_late),
    .id = "Area") %>% 
  mutate(
    mean_temp_1982_2009_F = as_fahrenheit(mean_temp_1982_2009_C, data_type = "temperature"),
    mean_anom_1982_2009_F = as_fahrenheit(mean_anom_1982_2009_C, data_type = "anomaly"),
    mean_temp_2010_2021_F = as_fahrenheit(mean_temp_2010_2021_C, data_type = "temperature"),
    mean_anom_2010_2021_F = as_fahrenheit(mean_anom_2010_2021_C, data_type = "anomaly"),
    across(where(is.numeric), round, 3))

sst_summary_table %>% knitr::kable()
...
decadal_folder <- cs_path(box_group = "mills", subfolder = "Projects/Decadal Variability/Graphics/SST")



# Saving these assets:
ggsave(plot = shelf_rate_plot, filename = str_c(decadal_folder, "ne_shelf_warming_rate.png"))
ggsave(plot = gom_rate_plot, filename = str_c(decadal_folder, "gom_epu_warming_rate.png"))
ggsave(plot = shelf_hw, filename = str_c(decadal_folder, "ne_shelf_heatwaves.png"))
ggsave(plot = gom_hw, filename = str_c(decadal_folder, "gom_epu_heatwaves.png"))
ggsave(plot = warming_rate_map, filename = str_c(decadal_folder, "warming_rates_map.png"))
write_csv(sst_summary_table, str_c(decadal_folder, "sst_area_warming.csv"))
...
age_increment_ccf %>% 
  ...
  facet_wrap(`Age Increment`~., labeller = as_labeller(~str_c("Age Increment ", .x)), 
             ncol = 2) +
