...
temp_details <- function(temp_dataset){
  ...
  annual_avgs <- temp_dataset %>% group_by(year) %>% summarise(sst = mean(area_wtd_sst), sst_anom = mean(area_wtd_anom))
...
area_summaries <- map(list("northeast shelf" = shelf_oisst, "Gulf of Maine" = gom_oisst, "Global Oceans" = global_oisst), temp_details)
...
ggplot() +
  geom_point(data = data_1$data_early, aes(year, sst), color = gmri_cols("blue")) +
  stat_smooth(data = data_1$data_early, method = "lm", formula = y ~ x, aes(year, sst), color = gmri_cols("blue")) +
  #geom_segment(aes(x = 1982, xend = 2009, y = data_1$mean_early, yend = data_1$mean_early), color = gmri_cols("blue"), linewidth = 1) +
  geom_point(data = data_1$data_late, aes(year, sst), color = gmri_cols("orange")) +
  #geom_segment(aes(x = 2010, xend = 2021, y = data_1$mean_late, yend = data_1$mean_late), color = gmri_cols("orange"), linewidth = 1) +
  stat_smooth(data = data_1$data_late, method = "lm", formula = y ~ x, aes(year, sst), color = gmri_cols("orange")) +
  labs(x = "Year", y = "SST", title = "Northeast Shelf SST Changes")
