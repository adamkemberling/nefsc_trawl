...
temp_regimes <- regional_oisst %>% 
  filter(yr > 1981) %>% 
  mutate(regime = ifelse(yr < 2010, "1982-2011 Regime", "2010-2021 Regime")) %>% 
  ...
  mutate(
    ...
    avg_anom_c = ifelse(regime == "1982-2011 Regime", 0, avg_anom_c),
    x = ifelse(regime == "1982-2011 Regime", 1981.5, 2009.5),
    xend = ifelse(regime == "1982-2011 Regime", 2009.5, 2021.5),
         ) %>% 
  ungroup()
...
temp_regimes %>% 
  ...
  labs(
    title = "Northwest-Atlantic Surface Temperature Regime Shift",
    x = "", 
...
regime_dat <- filter(nmfs_bio, est_year >= 2000)
...
plot_regime_sizes <- function(spec, var = "length", age_lim = 5, facet_prefix = "Age - ", facet_suffix = ""){
  ...
  filter(
    comname  == spec,
    age <= 6) %>% 
  ggplot(aes(x = !!{{var_col}}, group = regime, fill = regime)) +
...
regime_comparison %>% 
  ...
  # ggplot(aes(x = age, y = growth_shift)) +
  ggplot(aes(x = age, y = growth_frac)) +
    ...
    facet_wrap(~comname, scales = "free_x") + 
...
regime_comparison %>% 
  filter(age <= 5) %>% 
  ggplot(aes(x = age, y = fct_rev(comname))) +
  geom_tile(aes(fill = growth_frac)) +
  scale_fill_distiller(palette = "PRGn", limits = c(-.4, .4), na.value = "transparent") +
  labs(x = "Age", y = "Species", fill = "Proportional Shift in Length at Age")
```
...
regime_comparison %>% 
  filter(age <= 21) %>% 
  group_by(age, comname) %>% 
  count(size_change) %>% 
  ggplot(aes(x = age, y = n, fill = fct_rev(size_change), group = comname)) +
  geom_stream(
    type = "mirror", 
    color = "gray20", 
    size = 0.2, 
  ...
  labs(x = "Age", 
       y = "",
       title = "Size-at-Age Responses of 17 Groundfish Species\nChange in Growth During Warmer Temperature Regime", fill = "Change in Size at Age") +
  theme(
...
ggplot(data = waffle_iron(
  data = filter(regime_comparison, age == 3), 
  ...
  theme(axis.text = element_blank(),
        ...
        legend.position = "bottom",
        panel.grid.major.y = element_blank()) +
  labs(x = "", y = "", fill = "Warm Regime Impact on Body Length:")
