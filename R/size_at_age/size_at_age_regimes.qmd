...
# Isolate the Shelf and Gulf of Maine

# The Gulf of Maine Used:
...
ggplot() +
  geom_point(data = data_1$data_early, aes(year, sst_anom), color = gmri_cols("blue"), alpha = 0.4) +
  geom_point(data = data_1$data_late, aes(year, sst_anom), color = gmri_cols("orange"), alpha = 0.4) +
  ...
  geom_segment(aes(x = 1982, xend = 2009, y = data_1$mean_anom_early, yend = data_1$mean_anom_early),
               color = gmri_cols("blue"), linewidth = 1.2) +
  geom_segment(aes(x = 2010, xend = 2021, y = data_1$mean_anom_late, yend = data_1$mean_anom_late),
               color = gmri_cols("orange"), linewidth = 1.2) +
  annotate(x = 1996, y = 0.5, geom = "text", label = "Shelf-Wide Average\n1982-2009", color = gmri_cols("blue"), fontface = "bold") +
  annotate(x = 2016, y = 1.5, geom = "text", label = "Shelf-Wide Average\n2010-2021", color = gmri_cols("orange"), fontface = "bold") +
  annotate(x = 2027, y = 0.4, geom = "text", label = "Global Rate\n1982-2021", color = "black", fontface = "bold") +
  # stat_smooth(data = data_1$data_early, method = "lm", formula = y ~ x, aes(year, sst_anom),
  #             color = gmri_cols("blue"), fill = "transparent") +
  # stat_smooth(data = data_1$data_late, method = "lm", formula = y ~ x,
  #             aes(year, sst_anom), color = gmri_cols("orange"),  se = FALSE) +
  scale_y_continuous(labels = number_format(suffix = deg_c), 
                     expand = expansion(add = c(0.5,0.5))) +
  scale_x_continuous(expand = expansion(add = c(2,8))) +
  labs(x = "Year", y = "SST Anomaly", title = "Northeast Shelf SST Regime Shift")
...
what_you_have <- tribble(
  ~"station",  ~"comname", ~"depth", ~"biomass", ~"length", ~"numlen",
  "station_1", "redfish",  10,       25,         12,        2,
  "station_1", "redfish",  10,       25,         10,        1,
  "station_2", "redfish",  12,       25,         10,        2 
)


what_the_funciton_expects <- tribble(
  ~"station",  ~"comname", ~"depth", ~"biomass", ~"length", ~"numlen",
  "station_1", "redfish",  10,       25,         12,        2,
  #"station_1", "redfish",  10,       25,         10,        1,
  "station_2", "redfish",  12,       25,         10,        2 
...
age_increment_ccf %>% 
  ...
  facet_wrap(`Age Increment`~., labeller = as_labeller(~str_c("Age Increment ", .x)), ncol = 1) +
...
# K
ggplot(coef_and_cbio, aes(depth_mu, K)) +
  geom_smooth(
    method = "lm",
    formula = y ~ x,
    color = "gray50") +
...
# Linf
ggplot(coef_and_cbio, aes(depth_mu, Linf)) +
  geom_smooth(
    method = "lm",
    formula = y ~ x,
    color = "gray50") +
...
# K
ggplot(coef_and_cbio, aes(temp_mu, K)) +
  geom_smooth(
    method = "lm",
    formula = y ~ x,
    color = "gray50") +
...
# Linf
ggplot(coef_and_cbio, aes(temp_mu, Linf)) +
  geom_smooth(
    method = "lm",
    formula = y ~ x,
    color = "gray50") +
...
# K
ggplot(coef_and_cbio, aes(lat_mu, K)) +
  geom_smooth(
    method = "lm",
    formula = y ~ x,
    color = "gray50") +
...
# Linf
ggplot(coef_and_cbio, aes(lat_mu, Linf)) +
  geom_smooth(
    method = "lm",
    formula = y ~ x,
    color = "gray50") +
...
# K
ggplot(coef_and_cbio, aes(lon_mu, K)) +
  geom_smooth(
    method = "lm",
    formula = y ~ x,
    color = "gray50") +
...
# Linf
ggplot(coef_and_cbio, aes(lon_mu, Linf)) +
  geom_smooth(
    method = "lm",
    formula = y ~ x,
    color = "gray50") +
