...
plot_size_at_age <- function(comname_select, param = "weight"){
  ...
  # Put Last Five Years on Its Own plot
  plot_5yr <-  ggplot(last_five) +
    geom_line(aes(age, avg_weight, group = yearclass, color = fct_rev(factor(yearclass)))) +
    geom_dl(aes(age, avg_weight, label = yearclass, color = fct_rev(factor(yearclass))),
            method = list("last.points", directlabels::dl.trans(y = y + 0.75, x = x + 0))) +
     scale_color_grey() +
    scale_y_continuous(expand = expansion(mult = c(0, .2))) +
    labs(x = "Age", y = "Weight (kg)", 
         subtitle = "Averages for Last 5 Years",
         color = "Cohort Average")
  
  
  return((saa_plot / plot_5yr))
  
}


plot_size_at_age("atlantic cod", param = "weight")
#plot_size_at_age("acadian redfish", param = "weight")
