...
all_vonbert %>% 
  ...
  mutate(across(where(is.numeric), round, 1),
         age_range = NULL) %>% 
  setNames(
    c("Common Name", "Sex", "Decade", "# Aged", "# of Ages", "Age Min", "Age Max", "Avg. Fish / Age")) %>% DT::datatable()
...
## Changes in Size at Age - Raw Data
...
```{r}
# Pull a species and get average weight at age for each year
library(geomtextpath)
library(directlabels)

# Plotting a species changing weight at age:
plot_size_at_age <- function(comname, param = "weight"){
  
  #tester: comname <- "haddock"
  
  # Pull species
  this_species <- all_vonbert %>% 
  filter(comname == comname) %>% 
  ...
  # Get yearly averages
  yrly_avgs <- this_species %>% 
  group_by(est_year, age, yearclass, decade) %>% 
  summarise(avg_len = mean(length_cm, na.rm = T),
            avg_weight = mean(indwt, na.rm = T),
            .groups = "drop")
  
  # filter out last ten yrs
  last_five <- filter(yrly_avgs, between(yearclass, 2014, 2018))
  
  # last yr
  last_yr <- filter(yrly_avgs, yearclass == 2019)
  
  
  # Build Plots for decades
  this_species  %>% 
  ggplot(aes(age, indwt)) +
    ...
    geom_boxplot(width = .12, 
                 outlier.colour = NA,
                 alpha = 0.5) +
    geom_line(data = last_five, 
              aes(age, avg_weight, group = yearclass, color = "2014-2018")) +
    geom_dl(data = last_five, 
            aes(age, avg_weight, label = yearclass, color = "2014-2018"), method = "last.points") +
    geom_line(data = last_yr, 
              aes(age, avg_weight, color = "2019")) +
     geom_dl(data = last_yr, 
             aes(age, avg_weight, label = yearclass, color = "2019"), method = "last.points") +
     scale_color_gmri() +
    facet_wrap(~decade, ncol = 1) +
    ylim(c(0, 1)) +
    labs(x = "Age", y = "Weight (kg)", color = "Cohort Average",
         title = str_to_title(comname))
  
  
  # Build complicated plot with all years
# yrly_avgs %>% 
#    ggplot(aes(age, avg_weight, group = yearclass)) +
#      geom_boxplot(data = this_species, aes(age, indwt), color = "gray50", alpha = 0.5, outlier.color = NA) +
#     geom_line(data = last_five, aes(age, avg_weight, group = yearclass, color = "2014-2018")) +
#     geom_dl(data = last_five, aes(age, avg_weight, label = yearclass, color = "2014-2018"), method = "last.points") +
#     geom_line(data = last_yr, aes(age, avg_weight, color = "2019")) +
#      geom_dl(data = last_yr, aes(age, avg_weight, label = yearclass, color = "2019"), method = "last.points") +
#      scale_color_gmri() +
#      labs(x = "Age", y = "Weight (kg)", color = "Cohort", title = str_to_title(comname)) +
#      xlim(c(0,10)) +
#   ylim(0,5)
