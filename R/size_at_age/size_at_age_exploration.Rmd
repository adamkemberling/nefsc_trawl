...
Just looking at the raw data available, it becomes obvious that there are differences in the number of fish at various ages through time. With cases of large cohorts passing through, or older individuals being caught less frequently. Particularly large cohorts of haddock are a good example.
...
walk(key_species, plot_panelset, plot_fun = plot_age_bubbleplot)
...
plot_length_bubbles <- function(spec_choice){
  species_data[[spec_choice]] %>% 
    ...
    summarise(avg_len = mean(length_cm, na.rm = T),
              .groups = "drop") %>% 
    ggplot(aes(x = yearclass, y = age,  size = avg_len)) +
      geom_point(shape = 21, color = "white", fill = gmri_cols("gmri blue")) +
      facet_wrap(~comname) +
      theme(legend.position = "bottom") +
      labs(x = "Cohort", y = "Age", size = "Average Length (cm)") +
      guides(size = guide_legend(title.position = "top", title.hjust = 0.5))
}
...
plot_wt_bubbles <- function(spec_choice){
  species_data[[spec_choice]] %>% 
    ...
    summarise(avg_wt = mean(indwt, na.rm = T),
              .groups = "drop") %>% 
    ggplot(aes(x = yearclass, y = age,  size = avg_wt)) +
      geom_point(shape = 21, color = "white", fill = gmri_cols("gmri blue")) +
      facet_wrap(~comname) +
      scale_x_continuous(limits = c(1989, 2020)) +
      theme(legend.position = "bottom") +
      labs(x = "Cohort", y = "Age", size = "Average Weight (kg)") +
      guides(size = guide_legend(title.position = "top", title.hjust = 0.5))
}
...
# Code to switch variables and their labels:
  ...
  vb_incs %>% 
    ...
    ggplot(aes(est_year, {{var_sym}})) +
      ...
      geom_line(color = gmri_cols("gmri blue"), group = 1, alpha = 0.5, size = 1) +
      geom_smooth(method = "gam",
                  formula = y ~ s(x, bs = "cs", k = gam_knots),
                  se = FALSE,
                  size = 1,
...
## Plotting VB Coefficients {.panelset}
...
plot_vonbert_coef(comnames = c("atlantic cod",  "haddock", "winter flounder", "silver hake"), 
                  x_col = "five_yr_group", 
                  coef_name = "Linf")
...
## Age Abundance Ridgeplots 

I did these a while ago and am leaving it because they look cool. Deal.
...
For average age and average size by species the stratified abundances are used. Length is freely available, whereas age must be estimated by length. The view this gives us is how big the "average" individual is in the community.
